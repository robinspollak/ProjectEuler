...
matrix = map(lambda x: map(lambda y: int(y),x),matrix)

max_product = 0
for i in range(20):
	for j in range(16):
		prod1 = matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3]
		prod2 = matrix[j][i]*matrix[j+1][i]*matrix[j+2][i]*matrix[j+3][i]
		greater = max(prod1,prod2)
		if greater>max_product: max_product=greater
for i in range(16):
	for j in range(16):
